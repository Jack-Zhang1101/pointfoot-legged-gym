...
from .wheellegged_flat_config import BipedCfgWL
from legged_gym.utils.helpers import class_to_dict

class BipedWL(BaseTask):
    def __init__(
        self, cfg: BipedCfgWL, sim_params, physics_engine, sim_device, headless
    ):
        self.cfg = cfg
    ...
    def _compute_torques(self, actions):
        pos_action = (
            torch.cat(
                (
                    actions[:, 0:2], torch.zeros_like(actions[:, 0]).view(self.num_envs, 1),
                    actions[:, 3:5], torch.zeros_like(actions[:, 0]).view(self.num_envs, 1),
                ),
                axis=1,
            )
        ...
        vel_action = (
            torch.cat(
                (
                    torch.zeros_like(actions[:, 0:2]), actions[:, 2].view(self.num_envs, 1),
                    torch.zeros_like(actions[:, 0:2]), actions[:, 5].view(self.num_envs, 1),
                ),
                axis=1,
            )
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[16:] = 0.0  # previous actions
        return noise_vec
